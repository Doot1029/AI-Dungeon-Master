...
export const generateIllustration = async (prompt: string, isPgMode: boolean): Promise<string> => {
    const client = getAiClient();
    try {
        const fullPrompt = `An oil painting illustration of the following Dungeons and Dragons scene: ${prompt}. ${isPgMode ? 'The image must be strictly PG-rated and family-friendly, in a whimsical fantasy style.' : 'Dramatic, epic fantasy art style.'}`;
        ...
        throw new Error("Failed to generate an illustration.");
    }
};
